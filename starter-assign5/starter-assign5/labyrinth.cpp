// This file 'labyrinth.cpp' contains code regarding maze following and linked list structures.
#include "labyrinth.h"
 ...
 */
const string kYourName = "Ayush Raman";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "EESSENNSSWNWSWNSESW";

MazeCell* getAssociatedPointer(char direction, MazeCell* head) {
    char val = direction;
    if (val == 'E') {
        return head->east;
    } else if (val == 'S') {
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    for (int i = 0; i < moves.size(); i++) {
        ...
        MazeCell* next = getAssociatedPointer(moves[i], start);
        if (next != nullptr) {
            start = next;
        }
    }
    needs.remove(start->contents); // need this line to check the last node's content
    return needs.size() == 0;
}
