...
/*
 * This function expands the size of the array by 2
 */
void PQSortedArray::expand() {
    DataPoint* newArr = new DataPoint[_numAllocated * 2];
    for (int i = 0; i < _numFilled; i++) {
        newArr[i] = _elements[i];
    }
    delete [] _elements; // delete old reference
    _elements = newArr; // assign pointer to new array
    _numAllocated *= 2; // increase internal size
}

void PQSortedArray::moveOneUpFromIndex(DataPoint toFill) {
    for (int i = _numFilled - 1; i >= 0; i--) { // move right to left
        if (toFill.priority <= _elements[i].priority) { // if the priority is 'higher' than the current element, fill the element above it and return from the function
            _elements[i + 1] = toFill;
            return;
        } else {
            _elements[i + 1] = _elements[i]; // shift elements to the right
        }
    }
    _elements[0] = toFill; // in case we manage to loop through the entire array and still haven't found a higher priority, we can assume that 'toFill' has the highest priority.
}

/*
 * This function takes on argument of type DataPoint 'elem' and inserts it into the queue in a location based on the element's priority
 * The queue is sorted from highest to lowest priority
 */
void PQSortedArray::enqueue(DataPoint elem) {
    if (_numAllocated == _numFilled) {
        expand(); // expand the array if we need more memory
    }
    moveOneUpFromIndex(elem); // shift each element 1 to the right from the element to fill.
    _numFilled++; // increment the number filled
}
...
// Student Tests
STUDENT_TEST("PQSortedArray timing test, fillQueue and emptyQueue 20000") {
    PQSortedArray pq;
...
STUDENT_TEST("PQSortedArray timing test, fillQueue and emptyQueue 40000") {
    PQSortedArray pq;
    TIME_OPERATION(40000, fillQueue(pq, 40000));
    TIME_OPERATION(40000, emptyQueue(pq, 40000));
}

STUDENT_TEST("PQSortedArray timing test, fillQueue and emptyQueue 80000") {
    PQSortedArray pq;
    TIME_OPERATION(80000, fillQueue(pq, 80000));
    TIME_OPERATION(80000, emptyQueue(pq, 80000));
}
