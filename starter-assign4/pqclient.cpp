...
/* This function returns a vector of the top 'k' DataPoints from the 'stream'
 */
Vector<DataPoint> topK(istream& stream, int k) {
    Queue<DataPoint> inverted;
    PQHeap pq;
    ...
    while (stream >> point) {
        if (pq.size() < k) {
           ...
           while (!pq.isEmpty()) {
               DataPoint curr = pq.dequeue();
               if (curr.priority > point.priority) {
                   inverted.enqueue(curr);
               } else {
                   inverted.enqueue(point);
                   break;
               }
           }
           while (!inverted.isEmpty()) {
               pq.enqueue(inverted.dequeue());
           }
        }
   }
   while (!pq.isEmpty()) {
       vector.add(pq.dequeue());
   }
   vector.reverse();
   return vector;
}
...
STUDENT_TEST("topK: time trial, constant K") {
...
STUDENT_TEST("topK: time trial, constant n") {
    int startSize = 200000;
    int k;
    for (k = 10; k < startSize; k *= 2) {
        Vector<DataPoint> input;
        for (int i = 0; i < 1000; i++) {
            input.add({ "", randomInteger(1, 1000) });
        }
        stringstream stream = asStream(input);
